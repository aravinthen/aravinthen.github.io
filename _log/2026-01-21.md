...
\text{For each } s \in S: \\
\qquad \text{old-action} \leftarrow \pi(s) \\
\qquad \pi(s) \leftarrow \arg\max_a \sum_{s',r} p(s', r \mid s, a) \left[ r + \gamma V(s') \right] \\
\qquad \text{If old-action} \neq \pi(s) \text{, then policy-stable} \leftarrow \text{False} \\
...
\qquad\qquad\qquad Q(s, a) \leftarrow \sum_{s',r} p(s', r \mid s, a)
  \left[ r + \gamma \sum_{a'} \pi(a' \mid s')Q(s', a') \right] \\
\qquad\qquad\quad \Delta \leftarrow \max(\Delta, |q - Q(s,a)|) \\
\text{until } \Delta < \theta
\end{array}
$$,
...
Something that almost tripped me up as I was typing out this solution was the realisation that you don't need to speculate on the action when working with $$Q$$-functions - you already *have* the action via the formulation!
