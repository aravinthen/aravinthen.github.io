...
* The approach we saw in the previous paper was non-spectral, but nevertheless approximated the spectral approach by appealing to the representation of the filters as Chebyshev polynomial expansions. This is a approximation of the full graph convolutional network (GCN) in the paper by Kipf and Welling. 
...
* Results are either superior to or about as good as `GraphSAGE`, which is to be expected. What I'm getting here is that `GraphSAGE` is something of a inductive and localised version of convolution over graphs and `GAT` is an equivalent version that introduces an attention mechanism. *Both* of these methods are derived from the more fundamental GCN approach from Kipf and Welling, which is a global and transductive technique.
