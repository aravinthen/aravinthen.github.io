...
**Solution**:
Let's recall the policy algorithm described in the previous log. First initialise $$V(s) \in \mathbb{R}$$ and $$\pi(s) \in \mathcal{A}(s)$$ arbitrarily $$\forall s \in \mathcal{S}$$.
...
\qquad\qquad V &\leftarrow \sum_{s,r'} p(s', r \vert s, \pi(s)) \left[ r + \gamma V(s') \right] \\
\qquad\qquad \Delta &\leftarrow \text{max}(\delta, \lvert v - V(s)\rvert) \\
