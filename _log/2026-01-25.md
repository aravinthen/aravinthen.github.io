...
* Built a transformer as part of my weekly architecture implementation project.
...
3. A note on inputs: an embedding algorithm is used to provide an encoder layer a list of *word embeddedings* of length 512. Each encoder layer will subsequently provide a list of embeddings of the same length. Note that the length of the list is a hyperparameter, but is usually set to the longest sentence within the training dataset.
